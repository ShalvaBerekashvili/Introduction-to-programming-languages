...
void PrintTransposedMatrixInArray2D(int [,] array2D)
{
    for (int i = 0; i < array2D.GetLength(0); i ++)
    {
        for (int j = 0; j < array2D.GetLength(1); j ++)
...
int[,] TransposedMatrixInArray2D(int[,] array2D)
{
...
// array2D = TransposedMatrixInArray2D(array2D);
System.Console.WriteLine();
PrintTransposedMatrixInArray2D(array2D);